...
def test_update_fiche_address_only(repository, new_fiche):
    update_usecase = UpdateFicheUsecase(repository)
    updated_fiche = update_usecase(
        new_fiche.id, new_address="2 rue de Lyon, 69000 Lyon"
    )
...
def test_update_fiche_both(repository, new_fiche):
    ...
    updated_fiche = update_usecase(
        ...
        new_tel="9876543210",
        new_email="doejohn@gmail.com",
        new_address="2 rue de Lyon, 69000 Lyon",
    )
...
def test_cant_completion_fiche_works_empty(
    repository, new_fiche_empty_works, config_service
):
...
def test_cant_completion_fiche_not_found(repository, config_service):
    ...
    with pytest.raises(ValueError):
        completion_fiche_usecase(
            "id-not-existing", works_data=[{"work": "porte", "details": {}}]
        )
