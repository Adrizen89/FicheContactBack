...
def read_fiches_en_cours(repository: SQLiteFicheRepository = Depends(get_fiche_repository)):
...
def create_fiche(fiche: Fiche, usecase: CreateFicheUsecase = Depends(get_create_fiche_usecase)):
    try:
        logger.info("Creating new fiche", extra={
...
def add_travaux_to_fiche(
    ...
    try:
        logger.info("Adding works to fiche", extra={
            "fiche_id": fiche_id,
        ...
        logger.error("Validation error adding works", extra={"fiche_id": fiche_id, "error": str(e)})
        raise HTTPException(status_code=400, detail=str(e))
...
def update_fiche(fiche_id: str, fiche_update: Fiche, usecase: UpdateFicheUsecase = Depends(get_update_fiche_usecase)):
    try:
...
def validate_fiche(fiche_id: str, usecase: ValidateFicheUsecase = Depends(get_validate_fiche_usecase)):
    try:
        ...
        logger.error("Error validating fiche", extra={"fiche_id": fiche_id, "error": str(e)})
        raise HTTPException(status_code=400, detail=str(e))
...
def delete_fiche(fiche_id: str, usecase: DeleteFicheUsecase = Depends(get_delete_fiche_usecase)):
    try:
        ...
        logger.error("Error deleting fiche", extra={"fiche_id": fiche_id, "error": str(e)})
        raise HTTPException(status_code=400, detail=str(e))
